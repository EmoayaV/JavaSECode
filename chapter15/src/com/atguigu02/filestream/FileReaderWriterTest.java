...
//        }
    ...
    /*
     * 需求：读取hello.txt中的内容，显示在控制台上。(从磁盘中读取数据放到内存中)
     *
     * 对test2()进行优化，每次读取多个字符存放到字符数组中，减少了与磁盘交互的次数，提升效率。
     * 在内存上的时间复杂度和与磁盘交互的次数不是一个量级的，将时间复杂度从O(n2) ---> O(logn) 也许减少了0.1s的时间，但是与磁盘交互次数多一次也许就多了10s
     *
     * */
    @Test
    public void test3() {
        ...
        try {
            ...
            int len;
            while ((len = fr.read(cbuffer)) != -1) { //记录每次读入到cbuffer中的字符的个数
                //遍历数组:错误的写法
...
//                }
    ...
    /*
     * 需求：将内存中的数据写出到指定的文件中(从内存中读取数据放到磁盘中)
     * */
    @Test
    public void test4() {
        FileWriter fw = null;
...
//            fw = new FileWriter(file,true);
    ...
    /*
     * 需求：复制一份hello.txt文件，命名为hello_copy.txt(从磁盘中读取数据放到内存中，再从内存中读取数据放到磁盘中)
     * */
    @Test
    public void test5() {
        FileReader fr = null;
