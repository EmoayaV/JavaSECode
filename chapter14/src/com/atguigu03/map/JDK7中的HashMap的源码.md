...
void createEntry(int hash, K key, V value, int bucketIndex) {
    Entry<K,V> e = table[bucketIndex]; //e是原来数组位置i上的元素，如果数组位置i没有数据，那么e=null
    table[bucketIndex] = new Entry<>(hash, key, value, e); //新的元素的Entry指向旧的元素的Entry，如果原来数组位置i上的元素为null，那么新的元素的Entry指向null
    size++;
}
```
...
static class Entry<K,V> implements Map.Entry<K,V> {
    ...
    V value;
    Entry<K,V> next;
    int hash;  //使用key得到的哈希值2进行赋值。这里是作为一个属性进行保存了，就是说如果将来我们将key中的某个属性进行了改变，那么哈希值2不会发生改变
...
![image-20230527155604019](D:\markdown file\截图\image-20230527155604019.png)
