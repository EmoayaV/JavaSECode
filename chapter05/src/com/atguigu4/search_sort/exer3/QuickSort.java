...
public class QuickSort {
	...
	private static void subSort(int[] data, int start, int end) {
		if (start < end) { // 这个条件是结束递归的条件
			int base = data[start];
			int low = start;
			int high = end + 1;
			while (true) {
				while (low < end && data[++low] - base <= 0); //从左往右找，找到比base大的值的指针位置
				while (high > start && data[--high] - base >= 0); //从右往左找，找到比base小的值的指针位置
				if (low < high) {
			...
			//如果不满足start < high - 1则跳出递归
			subSort(data, start, high - 1);
			//通过递归调用，对data数组[high+1,end]部分的元素重复刚才的过程
			//如果不满足high + 1 < end 则跳出递归
